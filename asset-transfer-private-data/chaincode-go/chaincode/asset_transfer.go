...
func (s *SmartContract) UpdateAsset(ctx contractapi.TransactionContextInterface) error {
	...
	transientAssetJSON := transientMap["asset_properties"]

	var assetInput assetTransientInput
	json.Unmarshal(transientAssetJSON, &assetInput)
	...
	return ctx.GetStub().PutState(assetInput.ID, assetJSON)
}
...
func (s *SmartContract) CreateAsset(ctx contractapi.TransactionContextInterface) error {
	...
	if len(assetInput.Essay) == 0 {
		return fmt.Errorf("essay field must be a non-empty string")
	}
	if assetInput.Grade <= 0 {
		return fmt.Errorf("grade field must be a positive integer")
	}
...
func (s *SmartContract) TransferAsset(ctx contractapi.TransactionContextInterface) error {
	...
	asset.Owner = assetTransferInput.BuyerMSP
	...
	err = ctx.GetStub().PutState(assetTransferInput.ID, assetJSON)
	if err != nil {
		return err
	}
